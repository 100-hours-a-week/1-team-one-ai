...
"""
버전 공통 Task 스키마 (v2+)
- class UserInput(BaseModel)          — 추천 요청 공통 필드
- class ProgressStep(str, Enum)       — 진행 단계 열거형
- PROGRESS_STEP_PERCENTAGE            — 진행 단계별 퍼센트 매핑
- class TaskAcceptedResponse(BaseModel) — POST 202 즉시 응답
- class TaskResult(BaseModel)         — GET 폴링 / coreBE 콜백 페이로드
"""
...
class TaskAcceptedResponse(BaseModel):
    """
    POST /api/v{n}/routines 즉시 응답 (HTTP 202)
    - taskId와 초기 상태만 반환
    """
...
class TaskResult(BaseModel):
    ...
    1. polling: GET /api/v{n}/routines/{taskId} 폴링 응답
